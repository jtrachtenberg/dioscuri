...
add_text("MALORIAN ARMS  NIGHT CITY", 0.0075, 0.0220, 0.245, 0.046, "txt_nightcity")

# wordmark on the tube, ending before the dot logo
add_text("MALORIAN", 0.012, tx + 0.0004, 0.700, tz - 0.0055, "txt_malorian", spacing=1.15)

# dot logo at the muzzle edge, in front of the wordmark.
# 4 dots top row; bottom row missing the muzzle-most dot (mirrored ref).
DOT_Y0 = 0.780          # rearmost dot column (muzzle edge ~0.82)
DOT_STEP = 0.0060
for r in range(2):
    for c in range(4):
        if r == 1 and c == 3:
...
for name, (loc, rot) in views.items():
    ...
    scene.render.filepath = os.path.join(outdir, "v11_" + name + ".png")
    bpy.ops.render.render(write_still=True)
    print("RENDERED", name)
print("KITBASH V11 DONE")
