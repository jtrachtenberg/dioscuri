"""Dioscuri kitbash v5 — v4 composition + reference-photo styling:
gunmetal body, red grip, MALORIAN wordmark + dot matrix near muzzle,
patent text at barrel start, ash grip removed. Cycles render."""
import bpy
import bmesh
import math
import os
...
# ashura: carve out its own grip hiding inside the malorian one
for o in ashura:
    if o.type == 'MESH' and 'submesh_00' in o.name:
        n = trim(o, lambda co: co.z < -0.02 and co.y < 0.09)
        print(f"ASH GRIP CARVE: removed {n} verts")
...
for o in malorian:
    if o.type != 'MESH':
        continue
    o.data.materials.clear()
    o.data.materials.append(MAT_GUNMETAL)   # slot 0
    o.data.materials.append(MAT_RED_GRIP)   # slot 1
    bm = bmesh.new()
    bm.from_mesh(o.data)
    for f in bm.faces:
        c = f.calc_center_median()
        # local coords (pre-scale): grip zone = below frame line, rear of trigger guard
        f.material_index = 1 if (c.z < -0.018 and c.y < 0.035) else 0
    bm.to_mesh(o.data)
    bm.free()

# --- lettering (extruded text on the +X face of the barrel tube) ------
def add_text(body, size, y, z, name, spacing=1.0):
    curve = bpy.data.curves.new(name, 'FONT')
    curve.body = body
    curve.size = size
    curve.extrude = 0.0009
    curve.space_character = spacing
    obj = bpy.data.objects.new(name, curve)
    scene.collection.objects.link(obj)
    obj.rotation_euler = (math.pi / 2, 0, math.pi / 2)
    obj.location = (0.0225, y, z)
    obj.data.materials.append(MAT_TEXT)
    return obj

texts = []
texts.append(add_text("MALORIAN", 0.024, 0.60, 0.118, "txt_malorian", spacing=1.15))
texts.append(add_text("MALORIAN ARMS  NIGHT CITY", 0.008, 0.335, 0.125, "txt_patent1"))
texts.append(add_text("SILVERHAND CUSTOM [PAT.XIJS/001]", 0.008, 0.335, 0.112, "txt_patent2"))

# dot matrix behind the wordmark (3 x 6)
dots = []
for r in range(3):
    for c in range(6):
        bpy.ops.mesh.primitive_cylinder_add(radius=0.0022, depth=0.0018,
            location=(0.0225, 0.545 + c * 0.0065, 0.144 - r * 0.0075),
...
for name, (loc, rot) in views.items():
    ...
    scene.render.filepath = os.path.join(outdir, f"v5_{name}.png")
    bpy.ops.render.render(write_still=True)
    print("RENDERED", name)
print("KITBASH V5 DONE")
