...
p_tube = (0.0210, 0.0665)  # tube flank x / equator z (from full profile: tube z 0.047-0.090)
...
add_text("MALORIAN ARMS  NIGHT CITY", 0.0075, 0.0219, 0.305, 0.030, "txt_nightcity")

# wordmark on the tube, ending before the dot logo
add_text("MALORIAN", 0.012, tx + 0.0004, 0.672, tz - 0.0055, "txt_malorian", spacing=1.15)

# dot logo at the muzzle edge, in front of the wordmark.
# 4 dots top row; bottom row missing the muzzle-most dot (mirrored ref).
DOT_Y0 = 0.746          # rearmost dot column (clear of cap chamfer at ~0.776)
DOT_STEP = 0.0060
for r in range(2):
    for c in range(4):
        if r == 1 and c == 3:
...
views = {
    ...
    "detail":  ((L * 0.45, 0.70, 0.055), (math.radians(90), 0, math.radians(90))),
    "ref_view": ((-L * 1.35, cy, cz), (math.radians(90), 0, math.radians(-90))),
...
for name, (loc, rot) in views.items():
    ...
    scene.render.filepath = os.path.join(outdir, "v13_" + name + ".png")
    bpy.ops.render.render(write_still=True)
    print("RENDERED", name)
print("KITBASH V13 DONE")
