...
def add_text(body, size, x, y, z, name, spacing=1.0):
    ...
    so the left copy anchors at y + measured width."""
    right = make_text(body, size, x, y, z, name + "_R", math.pi / 2, spacing)
    bpy.context.view_layer.update()
    width = right.dimensions.y   # world AABB along the barrel axis
    left = make_text(body, size, -x, y + width, z, name + "_L", -math.pi / 2, spacing)
    return right, left
...
for name, (loc, rot) in views.items():
    ...
    scene.render.filepath = os.path.join(outdir, f"v17_{name}.png")
