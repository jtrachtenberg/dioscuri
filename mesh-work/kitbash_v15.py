...
# --- red grip: crease-bounded flood fill ------------------------------
mal_body = max((o for o in malorian if o.type == 'MESH'),
...
bm = bmesh.new()
bm.from_mesh(mal_body.data)
bm.faces.ensure_lookup_table()

# seed: outermost (-x) face in the grip's z-band = guaranteed outer shell
grip_faces = [f for f in bm.faces if f.calc_center_median().z < -0.06]
seed_face = min(grip_faces, key=lambda f: f.calc_center_median().x)

region = {seed_face.index}
...
while queue:
    ...
    for e in f.edges:
        ...
        for nf in e.link_faces:
            if nf.index not in region and nf.calc_center_median().z < 0.005:
                region.add(nf.index)
                queue.append(nf)

total = len(bm.faces)
rc = [bm.faces[i].calc_center_median() for i in region]
print(f"GRIP FLOOD: {len(region)}/{total} faces")
print("REGION x %.3f..%.3f y %.3f..%.3f z %.3f..%.3f" % (min(c.x for c in rc), max(c.x for c in rc), min(c.y for c in rc), max(c.y for c in rc), min(c.z for c in rc), max(c.z for c in rc)))
if len(region) > total * 0.6:
    print("GRIP FLOOD LEAKED - falling back to threshold mask")
    for f in bm.faces:
        c = f.calc_center_median()
        f.material_index = 1 if (c.z < -0.012 and c.y < 0.040) else 0
else:
    for f in bm.faces:
        f.material_index = 1 if f.index in region else 0
bm.to_mesh(mal_body.data)
bm.free()
