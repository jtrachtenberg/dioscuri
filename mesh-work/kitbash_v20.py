...
def make_text(body, size, x, y, z, name, rot_z, spacing=1.0):
...
def add_text(body, size, x, y, z, name, spacing=1.0):
    """Same lettering on both flanks, spanning the SAME y range.
    Right baseline runs +y from its origin; the left baseline runs -y,
    so the left copy anchors at the right text's true muzzle-side end,
    measured from the evaluated glyph mesh (dimensions is not yet
    populated on a freshly created FONT object)."""
    right = make_text(body, size, x, y, z, name + "_R", math.pi / 2, spacing)
    dg = bpy.context.evaluated_depsgraph_get()
    eo = right.evaluated_get(dg)
    m = eo.to_mesh()
    y_end = max((eo.matrix_world @ v.co).y for v in m.vertices)
    eo.to_mesh_clear()
    print(f"TEXT {name}: span {y:.3f}..{y_end:.3f}")
    left = make_text(body, size, -x, y_end, z, name + "_L", -math.pi / 2, spacing)
    return right, left

# upper receiver flat, rear of the barrel start (per red-M annotation)
add_text("MALORIAN ARMS  NIGHT CITY", 0.0075, 0.0220, 0.140, 0.059, "txt_nightcity")
# wordmark + dots eased back from the muzzle dip
add_text("MALORIAN", 0.012, tx + 0.0004, 0.655, tz - 0.0055, "txt_malorian", spacing=1.15)

DOT_Y0 = 0.737
DOT_STEP = 0.0060
for side_x in (tx + 0.0006, -(tx + 0.0006)):
    for r in range(2):
        for c in range(4):
            if r == 1 and c == 3:
                continue
            bpy.ops.mesh.primitive_cylinder_add(radius=0.0021, depth=0.0016,
                location=(side_x, DOT_Y0 + c * DOT_STEP, (tz + 0.0032) - r * 0.0064),
                rotation=(0, math.pi / 2, 0))
            d = bpy.context.active_object
            d.name = f"dot_{'R' if side_x > 0 else 'L'}_{r}_{c}"
...
views = {
    ...
    "left_logo": ((-0.405, 0.66, 0.055), (math.radians(90), 0, math.radians(-90))),
...
for name, (loc, rot) in views.items():
    ...
    scene.render.filepath = os.path.join(outdir, f"v20_{name}.png")
    bpy.ops.render.render(write_still=True)
    print("RENDERED", name)
print("KITBASH V20 DONE")
