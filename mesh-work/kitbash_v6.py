"""Dioscuri kitbash v6 — text anchored to measured surfaces, junk quads
...
for o in malorian:
    ...
    for f in bm.faces:
        ...
        f.material_index = 1 if (c.z < -0.012 and c.y < 0.045) else 0
    bm.to_mesh(o.data)
    bm.free()
...
p_rear = probe(0.28, 0.46, 0.00, 0.08)
...
rx, rz = p_rear
fx, fz = p_front
add_text("MALORIAN ARMS  NIGHT CITY", 0.0075, rx + 0.0005, 0.29, rz + 0.012, "txt_patent1")
add_text("SILVERHAND CUSTOM [PAT.XIJS/001]", 0.0075, rx + 0.0005, 0.29, rz + 0.002, "txt_patent2")
...
for name, (loc, rot) in views.items():
    ...
    scene.render.filepath = os.path.join(outdir, f"v6_{name}.png")
