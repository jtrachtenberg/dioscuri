"""Dioscuri kitbash v8 â€” branding corrections per reference:
- no SILVERHAND/patent line (Johnny-specific)
- MALORIAN ARMS NIGHT CITY just past the trigger section
- MALORIAN wordmark on the barrel tube toward the muzzle
- dot logo at the very muzzle edge, in front of the wordmark:
  4 dots top row, 3 bottom (missing the muzzle-most bottom dot)
- tighter red grip mask"""
import bpy
import bmesh
import math
import os
...
for o in malorian:
    ...
    for f in bm.faces:
        ...
        # tightened: only the grip proper (below frame, rear of trigger guard)
        f.material_index = 1 if (c.z < -0.020 and c.y < 0.030) else 0
    bm.to_mesh(o.data)
    bm.free()
...
for o in ashura + malorian:
    ...
    m = eo.to_mesh()
    world_verts += [eo.matrix_world @ v.co for v in m.vertices]
    eo.to_mesh_clear()
...
p_receiver = probe(0.16, 0.30, 0.00, 0.06)     # just past the trigger section
# tube side face: the max-x vertex in the span IS the tube equator
tube_sel = [w for w in world_verts if 0.60 <= w.y <= 0.72 and w.x > 0.005 and w.z > 0.06]
tube_v = max(tube_sel, key=lambda w: w.x)
p_tube = (tube_v.x, tube_v.z)
print("PROBE receiver:", p_receiver)
print("PROBE tube    :", p_tube)
...
# receiver text: just past the trigger
add_text("MALORIAN ARMS  NIGHT CITY", 0.0075, rx + 0.0005, 0.168, rz - 0.002, "txt_nightcity")

# wordmark on the tube, ending before the dot logo
add_text("MALORIAN", 0.014, tx - 0.0008, 0.615, tz - 0.007, "txt_malorian", spacing=1.15)

# dot logo at the muzzle edge, in front of the wordmark.
# 4 dots top row; bottom row missing the muzzle-most dot (mirrored ref).
DOT_Y0 = 0.728          # rearmost dot column
DOT_STEP = 0.0068
for r in range(2):
    for c in range(4):
        if r == 1 and c == 3:
            continue    # missing: bottom row, muzzle-most column
        bpy.ops.mesh.primitive_cylinder_add(radius=0.0021, depth=0.0016,
            location=(tx - 0.0004, DOT_Y0 + c * DOT_STEP, (tz + 0.0034) - r * 0.0068),
...
views = {
    ...
    "detail":  ((L * 0.55, 0.62, 0.10), (math.radians(90), 0, math.radians(90))),
...
for name, (loc, rot) in views.items():
    ...
    scene.render.filepath = os.path.join(outdir, "v10_" + name + ".png")
    bpy.ops.render.render(write_still=True)
    print("RENDERED", name)
print("KITBASH V10 DONE")
